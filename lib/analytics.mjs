...
export async function logSearch(q,total){
 ...
 try{
  ...
  const k=query.toLowerCase(),prev=doc.queries?.[k]||{query,count:0,zeroCount:0,totalResultSum:0,lastTotal:0};
  doc.queries={...(doc.queries||{}),[k]:{query:prev.query||query,count:Number(prev.count||0)+1,zeroCount:Number(prev.zeroCount||0)+(Number(total||0)===0?1:0),totalResultSum:Number(prev.totalResultSum||0)+Number(total||0),lastTotal:Number(total||0),lastAt:new Date().toISOString()}};
...
export async function readAnalytics(days=30){
 ...
 for(const doc of docs){for(const v of Object.values(doc?.queries||{})){const k=String(v.query||'').toLowerCase();const x=out.get(k)||{query:v.query||k,count:0,zeroCount:0,totalResultSum:0,lastTotal:0,lastAt:null};x.count+=Number(v.count||0);x.zeroCount+=Number(v.zeroCount||0);x.totalResultSum+=Number(v.totalResultSum||0);if(!x.lastAt||String(v.lastAt||'')>x.lastAt){x.lastAt=v.lastAt||null;x.lastTotal=Number(v.lastTotal||0);}out.set(k,x);}}
