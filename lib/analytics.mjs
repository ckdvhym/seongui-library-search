...
export async function readAnalytics(days=30,{startDate=''}={}){
 const out=new Map(),now=new Date();const paths=[];
 for(let i=0;i<days;i++){const d=new Date(now);d.setUTCDate(d.getUTCDate()-i);const day=dayKey(d);if(startDate&&day<startDate)continue;paths.push(analyticsDayPath(day));}
