...
function safeList(v={},prev={}){return {id:s(v.id||prev.id||id()),name:s(v.name||prev.name).slice(0,100),type:TYPES.includes(s(v.type))?s(v.type):(TYPES.includes(prev.type)?prev.type:'기타'),description:s(v.description??prev.description).slice(0,400),startDate:dateOk(v.startDate)?v.startDate:(prev.startDate||''),endDate:dateOk(v.endDate)?v.endDate:(prev.endDate||''),active:v.active===undefined?(prev.active??true):!!v.active,featured:v.featured===undefined?(prev.featured??true):!!v.featured,items:Array.isArray(prev.items)?prev.items:[],createdAt:prev.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};}
...
export default async function handler(req,res){
 try{
  ...
  if(req.method==='GET'&&mode==='home'){
   ...
   let lists=(doc.lists||[]).filter(x=>isVisible(x)&&x.featured);
   if(!lists.length){res.setHeader('Cache-Control','public, s-maxage=30, stale-while-revalidate=180');return res.status(200).json({ok:true,lists:[]});}
   lists=await hydrateLists(lists);
   const seed=s(req.query?.seed||'home');
   const selected=pickBySeed(lists,2,seed).map(l=>{const totalItems=(l.items||[]).length;return {...l,totalItems,hasMore:totalItems>5,items:pickBySeed(l.items,5,`${seed}|${l.id}`)};});
   res.setHeader('Cache-Control','public, max-age=0, s-maxage=30, stale-while-revalidate=180');
  ...
  if(req.method==='GET'&&mode==='active'){
   const doc=await readJson(PATHS.recommendations,EMPTY,{ttlMs:60*1000});const lists=await hydrateLists((doc.lists||[]).filter(isVisible));return res.status(200).json({ok:true,lists});
  }
  ...
  if(req.method==='GET'&&mode==='admin'){
   if(!requireAdmin(req,res))return;const doc=await readJson(PATHS.recommendations,EMPTY);return res.status(200).json({ok:true,types:TYPES,lists:doc.lists||[]});
  }
