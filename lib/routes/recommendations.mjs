...
export default async function handler(req,res){
 try{
  ...
  if(req.method==='GET'&&mode==='admin'){
   if(!requireAdmin(req,res))return;res.setHeader('Cache-Control','no-store, max-age=0');const doc=await readJson(PATHS.recommendations,EMPTY);return res.status(200).json({ok:true,types:TYPES,lists:doc.lists||[]});
  }
