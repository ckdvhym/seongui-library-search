...
형식 {"semanticKind":"topic","coreKeywords":[],"contextKeywords":[],"softTopics":[],"softKeywords":[]}\n검색어: ${q}`;
  ...
  const core=[original,...(x.coreKeywords||[])].filter(Boolean);
  const patch={softTopics:[...new Set(x.softTopics||[])].slice(0,3),softKeywords:[...new Set(x.softKeywords||[])].slice(0,8),semanticKind:String(x.semanticKind||'other'),semanticCoreKeywords:[...new Set(core)].slice(0,8),semanticContextKeywords:[...new Set(x.contextKeywords||[])].slice(0,6),semanticFallback:!!forceSemantic};
