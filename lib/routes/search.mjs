...
export async function runSearch(q,idx,{useAI=true,offset=0,limit=12,withSuggestions=false}={}){
 ...
 // 정확한 자료가 한 권이라도 있으면 수를 채우려고 약한 결과를 섞지 않는다.
 // 정확 결과가 0건일 때에만 단계적으로 넓혀 '관련성보다 결과 수가 앞서는' 퇴행을 막는다.
 if(hits.length===0)merge(1);
 if(hits.length===0)merge(2);
 // 복합 주제는 앞 단계에서도 0건일 때만 마지막 관련자료 안전망을 사용한다.
 if(hits.length===0&&(intent.conceptGroups||[]).length>1)merge(3);
...
async function buildSuggestions(q,idx){
 ...
 for(const x of verified.sort((a,b)=>b.count-a.count))if(!seen.has(norm(x.query))){seen.add(norm(x.query));out.push(x);if(out.length>=5)break;}
