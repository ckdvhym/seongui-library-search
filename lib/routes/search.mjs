...
async function buildSuggestions(q,idx){
 ...
 // 관련 주제는 원 검색 결과가 약할 때 학생을 실제 소장자료가 있는 인접 주제로 연결하는 안전망이다.
 // 특정 검색어를 하드코딩하지 않고, 해석된 주제 + 상위 결과의 공통 태그에서 후보를 만든 뒤 실제 장서 검색으로 검증한다.
 const base=await runSearch(q,idx,{useAI:true,offset:0,limit:18,withSuggestions:false});
 const intent=base.intent||interpretLocal(q),candidates=refinementCandidates(q,intent),nq=norm(q);
 const add=x=>{if(x&&norm(x)!==nq&&!candidates.includes(x))candidates.push(x);};
 for(const t of intent.softTopics||[])add(SUGGEST_TOPIC_QUERY[t]);
 const freq=new Map();for(const r of (base.results||[]).slice(0,10)){for(const t of r.topics||[])if(SUGGEST_TOPIC_QUERY[t])freq.set(SUGGEST_TOPIC_QUERY[t],(freq.get(SUGGEST_TOPIC_QUERY[t])||0)+1);for(const g of r.genres||[])if(SUGGEST_GENRE_QUERY[g])freq.set(SUGGEST_GENRE_QUERY[g],(freq.get(SUGGEST_GENRE_QUERY[g])||0)+1);}
 for(const [x] of [...freq.entries()].sort((a,b)=>b[1]-a[1]).slice(0,6))add(x);
 // 로컬 사전만으로 인접 주제가 부족한 낯선 표현은 Gemini가 '검색어 후보'만 제안한다.
 // 실제 학생 화면에는 우리 장서에서 결과가 검증된 후보만 남는다.
 if(candidates.length<3){for(const x of await geminiRefinements(q,intent))add(x);}
 const checked=candidates.slice(0,12);if(!checked.length){setSuggestCached(q,idx,[]);return [];}
 const verified=await Promise.all(checked.map(async query=>{const x=await runSearch(query,idx,{useAI:false,offset:0,limit:1,withSuggestions:false});return x.total>=1?{query,count:x.total}:null;}));
