...
export async function runSearch(q,idx,{useAI=true,offset=0,limit=12,withSuggestions=false}={}){
 ...
 const aiNeeded=useAI&&!exact&&(semanticFallback||(intent.majorIntent&&!intent.majorArea)||(exactHits.length<4&&needsSemanticAI(intent,{exact,semanticFallback}))||(intent.complex&&exactHits.length<4));
 if(aiNeeded){
...
export async function buildSuggestions(q,idx){
 ...
 // 관련 주제는 본 검색을 막지 않는 별도 비동기 경로다. 먼저 로컬 후보를 실제 장서로 검증하고,
 // 충분하지 않을 때만 Gemini가 인접 주제 후보를 보태며 최종 노출 전 다시 장서 결과를 확인한다.
 const base=await runSearch(q,idx,{useAI:false,offset:0,limit:18,withSuggestions:false});
 const intent=base.intent||interpretLocal(q),candidates=refinementCandidates(q,intent),nq=norm(q);
 const add=x=>{x=String(x||'').trim();if(x&&norm(x)!==nq&&!candidates.includes(x))candidates.push(x);};
 for(const t of intent.softTopics||[])add(SUGGEST_TOPIC_QUERY[t]);
 const freq=new Map();
 for(const r of (base.results||[]).slice(0,12)){
  for(const t of r.topics||[])if(SUGGEST_TOPIC_QUERY[t])freq.set(SUGGEST_TOPIC_QUERY[t],(freq.get(SUGGEST_TOPIC_QUERY[t])||0)+1);
  for(const g of r.genres||[])if(SUGGEST_GENRE_QUERY[g])freq.set(SUGGEST_GENRE_QUERY[g],(freq.get(SUGGEST_GENRE_QUERY[g])||0)+1);
 }
 for(const [x] of [...freq.entries()].sort((a,b)=>b[1]-a[1]).slice(0,8))add(x);
 const verify=async list=>(await Promise.all(list.map(async query=>{const x=await runSearch(query,idx,{useAI:false,offset:0,limit:1,withSuggestions:false});return x.total>=1?{query,count:x.total}:null;}))).filter(Boolean);
 let verified=await verify(candidates.slice(0,12));
 // 로컬 태그가 너무 일반적이거나 검색어가 낯설어 실제 후보가 2개 미만이면 AI는 '후보 생성'에만 사용한다.
 // 검색 결과 자체는 기다리지 않으며, AI 후보도 소장자료가 실제 있는 경우에만 학생에게 보인다.
 if(verified.length<2){
  const before=new Set(candidates);
  for(const x of await geminiRefinements(q,intent))add(x);
  const extra=candidates.filter(x=>!before.has(x)).slice(0,8);
  if(extra.length)verified=[...verified,...await verify(extra)];
 }
 const seen=new Set(),out=[];
 // 후보 생성 순서가 의미 우선순위다. 권수 많은 넓은 주제가 앞을 차지하지 않도록 순서를 유지한다.
 for(const x of verified)if(!seen.has(norm(x.query))){seen.add(norm(x.query));out.push(x);if(out.length>=5)break;}
...
export default async function handler(req,res){
 ...
 try{
  ...
  const {intent,results,total,hasMore,nextOffset,notice,expanded,exactCount}=await runSearch(q,idx,{useAI:true,offset,limit,withSuggestions:false});
