...
export async function runSearch(q,idx,{useAI=true,offset=0,limit=12,withSuggestions=false}={}){
 ...
 if(withSuggestions&&offset===0&&hits.length){
  const freq=new Map();for(const r of hits.slice(0,24)){for(const t of r.primaryTopics?.length?r.primaryTopics:(r.topics||[])){const x=SUGGEST_TOPIC_QUERY[t];if(x)freq.set(x,(freq.get(x)||0)+1);}for(const g of r.genres||[]){const x=SUGGEST_GENRE_QUERY[g];if(x)freq.set(x,(freq.get(x)||0)+1);}}
  const nq=norm(q),candidates=[...freq.entries()].filter(([x])=>norm(x)!==nq).sort((a,b)=>b[1]-a[1]).slice(0,7).map(x=>x[0]);
  const verified=await Promise.all(candidates.map(async query=>{const x=await runSearch(query,idx,{useAI:false,offset:0,limit:1,withSuggestions:false});return x.total>=2?{query,count:x.total}:null;}));suggestions=verified.filter(Boolean).slice(0,5);
 }
 return {intent,results,total,hasMore:offset+results.length<total,nextOffset:offset+results.length,suggestions};
}
