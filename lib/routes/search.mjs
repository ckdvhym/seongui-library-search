...
const CURATED_REFINEMENTS={
 ...
 '건축·도시':['도시와 건축','건축 진로','친환경 건축','건축 디자인'],
...
const CONTEXT_REFINEMENTS={
 urban:['도시와 건축','도시계획','지역 재생','친환경 도시','공공 공간'],
 architecture:['건축 진로','도시와 건축','친환경 건축','건축 디자인','공간 디자인'],
 health:['공중보건','보건의료','예방의학','건강정책','의료윤리']
};
function refinementCandidates(q,intent){
 const nq=norm(q),keys=[],out=[];
 const add=x=>{if(x&&norm(x)!==nq&&!out.includes(x))out.push(x);};
 if(/도시|재생|지역개발|도시계획/.test(nq))for(const x of CONTEXT_REFINEMENTS.urban)add(x);
 else if(/건축|공간|주거/.test(nq))for(const x of CONTEXT_REFINEMENTS.architecture)add(x);
 if(/보건|공중보건|건강정책|보건의료/.test(nq))for(const x of CONTEXT_REFINEMENTS.health)add(x);
 ...
 if(/인공지능|생성형ai|\bai\b/i.test(String(q||'')))keys.unshift('인공지능·정보');
 if(/기후|환경|탄소/.test(nq))keys.unshift('환경·기후');
 for(const k of keys)for(const x of CURATED_REFINEMENTS[k]||[])add(x);
...
function needsSemanticAI(base,{exact=false,semanticFallback=false}={}){
 ...
 const phraseGroup=(base.conceptGroups||[]).length===1&&base.conceptGroups[0]?.source==='phrase';
 if(phraseGroup&&base.conceptGroups[0]?.topic)return false; // 로컬에서 의미 분야를 이미 아는 복합 구문은 AI 대기 없이 처리
...
function broadTasteKind(q){const n=norm(q);if(/^(재미있는|재밌는|흥미로운)(책|도서|소설)?$/.test(n)||/재미있는책|재밌는책|흥미로운책/.test(n))return 'fun';if(/가볍게읽|술술읽|부담없이읽/.test(n))return 'light';return '';}
function tasteSearch(kind,idx){
 const preferred=new Set(['추리·미스터리','SF','판타지','로맨스','성장소설','공포','유머']);
 const words=kind==='light'?['가볍','술술','짧은','단편','유쾌','따뜻','재미']:['재미','흥미','유쾌','웃음','기발','모험','반전','빠른 전개','몰입'];
 const out=[];
 for(const r of (idx.records||[])){
  const k=String(r.kdc||''),mt=String(r.materialType||''),genres=r.genres||[],text=[r.title,r.introPreview,(r.keywords||[]).join(' ')].join(' ');
  const literary=/^8/.test(k)||['소설','문학','에세이'].includes(mt)||genres.some(g=>preferred.has(g));if(!literary)continue;
  let score=0,reasons=[];for(const g of genres){if(preferred.has(g)){score+=6;reasons.push(g);}}
  for(const w of words){if(String(r.title||'').includes(w)){score+=10;reasons.push(w);}else if(String(text).includes(w)){score+=3;}}
  if(mt==='소설'||/^8\d3/.test(k)){score+=4;reasons.push('소설');}
  if(kind==='light'&&(/단편|에세이/.test(text)||mt==='에세이'))score+=4;
  if(score<4)continue;out.push({...r,_score:score,_reasons:[...new Set(reasons)].slice(0,4),_relaxLevel:0});
 }
 out.sort((a,b)=>b._score-a._score||String(b.year||'').localeCompare(String(a.year||'')));return out;
}
export async function runSearch(q,idx,{useAI=true,offset=0,limit=12,withSuggestions=false}={}){
 let intent=interpretLocal(q);
 const tasteKind=broadTasteKind(q);
 ...
 let exactHits=tasteKind?tasteSearch(tasteKind,idx):collect(0,intent);
 // 2) 로컬만으로 결과가 부족하거나, 로컬 사전에 없는 개념/학과이면 질문 전체를 Gemini가 1회 해석한다.
 //    책별 AI 호출은 없으며 Gemini 실패 시에도 로컬 검색은 그대로 동작한다.
 const aiNeeded=!tasteKind&&useAI&&!exact&&(semanticFallback||(intent.majorIntent&&!intent.majorArea)||(exactHits.length<4&&needsSemanticAI(intent,{exact,semanticFallback})));
 if(aiNeeded){
 ...
 const merge=(level,maxAdd=Infinity)=>{const seen=new Set(hits.map(x=>x.id));let added=0;for(const r of collect(level,intent)){if(seen.has(r.id))continue;hits.push(r);seen.add(r.id);added++;if(added>=maxAdd)break;}hits.sort((a,b)=>a._relaxLevel-b._relaxLevel||b._score-a._score||String(b.year).localeCompare(String(a.year)));};
 const phraseGroup=(intent.conceptGroups||[]).length===1&&intent.conceptGroups[0]?.source==='phrase';
 // 정확 결과가 1권뿐인 전문 구문은 '정확 1권 + 같은 의미 분야의 가까운 자료 최대 3권'까지만 보완한다.
 // 도시 재생처럼 장서 표현이 조금 다른 주제를 살리되, 결과 수를 위해 넓은 책을 무더기로 섞지는 않는다.
 if(hits.length===1&&phraseGroup&&intent.conceptGroups[0]?.topic)merge(1,3);
 // 정확 결과가 전혀 없을 때만 단계적으로 넓힌다.
 if(hits.length===0)merge(1,6);
 if(hits.length===0)merge(2,6);
 if(hits.length===0&&(intent.conceptGroups||[]).length>1)merge(3,6);
...
async function geminiRefinements(q,intent){
 const key=process.env.GEMINI_API_KEY;if(!key)return[];
 try{
  const prompt=`학교도서관의 '관련 주제로 더 찾아보기' 버튼용 검색어를 만든다. 원 검색어의 핵심 주제를 유지하면서, 학생이 다음 탐색으로 눌러볼 만한 더 구체적이거나 인접한 주제 4~6개만 JSON 배열로 반환하라. 검색 목적 표현(책, 도서, 추천, 학생, 수행평가, 진로, 관련)은 반복하지 않는다. 너무 넓은 상위개념이나 원 주제와 먼 분야는 금지한다. 예: 도시 재생 → [도시와 건축, 도시계획, 지역 재생, 공공 공간]. 예: 보건 → [공중보건, 보건의료, 예방의학, 건강정책]. 형식 {"queries":["..."]}. 검색어: ${q}`;
  const u=`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite:generateContent?key=${encodeURIComponent(key)}`;
  const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),1700);let r;try{r=await fetch(u,{method:'POST',signal:ctrl.signal,headers:{'content-type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.1,responseMimeType:'application/json'}})});}finally{clearTimeout(timer);}if(!r.ok)return[];const j=await r.json(),text=j?.candidates?.[0]?.content?.parts?.[0]?.text;if(!text)return[];const x=JSON.parse(text);return [...new Set((x.queries||[]).map(v=>String(v||'').trim()).filter(Boolean))].slice(0,6);
 }catch{return[];}
}

function quickSuggestionExists(query,idx){
 const intent=interpretLocal(query);
 // 관련주제 검증은 결과 목록 전체를 만들거나 정렬하지 않고,
 // 실제 장서에서 한 권이라도 매칭되는 순간 끝낸다. 본 검색과 완전히 분리된 빠른 경로다.
 for(const level of [0,1]){
  const pass={...intent,relaxLevel:level};
  for(const r of (idx.records||[])){if(scoreRecord(r,pass))return true;}
 }
 return false;
}

export async function buildSuggestions(q,idx){
 const cached=getSuggestCached(q,idx);if(cached)return cached;
 const intent=interpretLocal(q),candidates=refinementCandidates(q,intent),nq=norm(q);
 const add=x=>{x=String(x||'').trim();if(x&&norm(x)!==nq&&!candidates.includes(x))candidates.push(x);};
 // 로컬에서 이미 아는 주제는 사전에 검토한 관련주제를 최우선으로 사용한다.
 for(const t of intent.softTopics||intent.topics||[])add(SUGGEST_TOPIC_QUERY[t]);
 // 관련주제 표시 경로는 '라벨 검증'까지만 한다.
 // 결과 페이지까지 여기서 미리 계산하면 관련주제 영역 자체가 늦게 나타나므로 금지한다.
 let verified=[];
 for(const query of candidates.slice(0,12)){
  if(quickSuggestionExists(query,idx))verified.push({query});
  if(verified.length>=5)break;
 }
 // 사전 후보가 거의 없는 낯선 검색어에서만 Gemini를 보조적으로 사용한다.
 // 이미 관련주제가 2개 이상 있으면 외부 API를 전혀 기다리지 않는다.
 if(verified.length<2){
  const extra=await geminiRefinements(q,intent);
  for(const query of extra){
   if(verified.some(x=>norm(x.query)===norm(query)))continue;
   if(quickSuggestionExists(query,idx))verified.push({query});
   if(verified.length>=5)break;
  }
 }
 const out=verified.slice(0,5);
 setSuggestCached(q,idx,out);return out;
}
...
export default async function handler(req,res){
 ...
 try{
  // 주제검색 화면에 들어온 직후 백그라운드에서 한 번 호출해 서버리스 함수와 검색 인덱스를 데운다.
  // 사용자가 첫 검색을 누를 때 발생하던 cold-start 체감을 줄이기 위한 경로다.
  const idx=await readJson(PATHS.searchIndex,null,{ttlMs:30*60*1000,blobCache:true});
  if(!idx?.records)return res.status(503).json({ok:false,message:'아직 검색 인덱스가 준비되지 않았습니다. 관리자에서 통합 구축을 먼저 실행해 주세요.'});
  if(mode==='warm'){res.setHeader('Cache-Control','no-store');return res.status(200).json({ok:true,warmed:true,version:idx.version||'unknown'});}
  if(!q)return res.status(400).json({ok:false,message:'검색어를 입력해 주세요.'});
  res.setHeader('Cache-Control','public, s-maxage=120, stale-while-revalidate=600');
  if(mode==='suggestions'){const suggestions=await buildSuggestions(q,idx);return res.status(200).json({ok:true,query:q,suggestions});}
  // 관련주제 버튼은 이미 로컬 장서 검증을 통과한 검색어이므로 AI 해석을 다시 기다리지 않는다.
  const useAI=String(req.query?.fromSuggestion||'')!=='1';
  const {intent,results,total,hasMore,nextOffset,notice,expanded,exactCount}=await runSearch(q,idx,{useAI,offset,limit,withSuggestions:false});
  return res.status(200).json({ok:true,version:idx.version||'unknown',query:q,intent:{majorIntent:!!intent.majorIntent,majorArea:intent.majorArea||null,material:intent.material,genres:intent.genres,topics:intent.topics,emotions:intent.emotions,origin:intent.origin,classicRequired:intent.classicRequired,classicTarget:intent.classicTarget||null,philosophyTradition:intent.philosophyTradition,requiredTerms:intent.requiredTerms,explicitConcepts:(intent.explicitConcepts||[]).map(x=>x.name),compoundConcepts:!!intent.compoundConcepts,aiInterpreted:!!intent.aiInterpreted,semanticKind:intent.semanticKind||null,semanticCoreKeywords:intent.semanticCoreKeywords||[],semanticDirectKeywords:intent.semanticDirectKeywords||[],semanticRelatedKeywords:intent.semanticRelatedKeywords||[]},count:results.length,total,exactCount,expanded,notice,hasMore,nextOffset,suggestions:[],results});
