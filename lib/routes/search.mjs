...
async function buildSuggestions(q,idx){
 ...
 const verified=await Promise.all(candidates.map(async query=>{const x=await runSearch(query,idx,{useAI:false,offset:0,limit:1,withSuggestions:false});return x.total>=2?{query,count:x.total}:null;}));
