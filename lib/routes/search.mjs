...
export async function buildSuggestions(q,idx){
 ...
 // 후보별 전체 검색을 반복하지 않고 '실제 장서 1권 이상 존재 여부'만 빠르게 확인한다.
 let verified=[];
 ...
 for(const item of verified.slice(0,5)){
  const page=await runSearch(item.query,idx,{useAI:false,offset:0,limit:12,withSuggestions:false});
  out.push({query:item.query,page:{results:page.results,total:page.total,exactCount:page.exactCount,expanded:page.expanded,notice:page.notice,hasMore:page.hasMore,nextOffset:page.nextOffset}});
 }
