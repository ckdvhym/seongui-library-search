...
export default async function handler(req,res){
 ...
 try{
   const id=String(req.query?.id||''),chunk=Number(req.query?.chunk),requestIsbn=cleanIsbn(req.query?.isbn),query=String(req.query?.q||'').trim(),fast=String(req.query?.fast||'')==='1';
   ...
   if(!fast&&needsRepair&&!hasUseful(y)&&query&&r?.title&&norm(query)===norm(r.title)){
