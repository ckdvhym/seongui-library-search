...
function audit(records){
 const cases=['중국 소설','기후 위기와 관련된 한국 소설','서양 철학','동양 철학','고전 소설','고전 한국 문학','환경과 관련 있는 소설','침묵의 봄','사회 문제를 다룬 책','힐링을 원할 때','재미있는 책','로맨스 책','로맨스 관련 책','로맨스 관련 소설','슬픈 소설','학교를 배경으로 하는 소설','백제 역사 책','미국 전쟁을 보여주는 책','생명과학과 정보가 연계된 책','돈 관련 책','재테크 관련 책','연산군','프랑스 전쟁','인생에 대해 돌아볼 수 있는 책','인공지능과 환경 오염','유전공학과 윤리','자유주의와 경제 격차','수의사 관련 책','생명공학 진로 책'];
 const out=[];
...
function makeRecord(b,idx,m){
 ...
 return {id:b.id,chunk:Math.floor(idx/META_BATCH),title:b.identity?.title||'',author:b.identity?.author||'',publisher:b.identity?.publisher||'',year:b.identity?.year||'',isbn13:b.identity?.isbn13||'',copies:b.holding?.copies||1,callNumbers:b.holding?.callNumbers||[],locations:b.holding?.locations||[],latestRegistrationDate:b.holding?.latestRegistrationDate||'',kdc:b.holding?.kdc||'',materialType:prof.materialType||null,materialTypes:prof.materialTypes||[],genres:prof.genres||[],primaryTopics:prof.primaryTopics||[],secondaryTopics:prof.secondaryTopics||[],topics:prof.topics||[],topicEvidence:prof.topicEvidence||{},emotions:prof.emotions||[],literatureOrigin:prof.literatureOrigin||null,originScores:prof.originScores||{},philosophyScores:prof.philosophyScores||{},classicScore:prof.classicScore||0,classicMeta:!!prof.classicMeta,keywords:prof.keywords||[],tocTokens:prof.tocTokens||[],cover:m?.yes24?.cover||null,hasCover:!!m?.yes24?.cover,hasIntro:!!intro,hasToc:!!m?.yes24?.toc,introPreview:intro.slice(0,700),metadataStatus:m?.yes24?.status||'base'};
}
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(action==='reset'){
   await writeJson(PATHS.searchBuildState,{version:'V8.3',processed:0,total,completed:false,startedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,processed:0,total,completed:false,message:'검색 인덱스 재구축을 시작합니다.'});
  }

  if(action==='step'){
   let st=await readJson(PATHS.searchBuildState,{version:'V8.3',processed:0,total,completed:false});
   if(st.version!=='V8.3'||st.total!==total){st={version:'V8.3',processed:0,total,completed:false,startedAt:new Date().toISOString()};}
   ...
   await writeJson(searchChunkPath(part),{version:'V8.3',part,start,end,records});
  ...
  if(action==='finalize'){
   ...
   await writeJson(PATHS.searchIndex,{version:'V8.3',builtAt:new Date().toISOString(),schoolId:catalog.schoolId||'seongui-high',bookCount:records.length,metadataProcessed:p.processed||0,audit:auditSummary,records});
   const state=await readJson(PATHS.state,{});state.search={ready:true,builtAt:new Date().toISOString(),bookCount:records.length,metadataProcessed:p.processed||0,version:'V8.3',audit:auditSummary};await writeJson(PATHS.state,state);
   await writeJson(PATHS.searchBuildState,{...st,finalized:true,finalizedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,version:'V8.3',bookCount:records.length,audit:auditSummary,message:`검색 인덱스 ${records.length.toLocaleString()}종을 V8.3 기준으로 재구축했습니다.`});
  }
