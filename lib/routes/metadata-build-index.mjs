...
export default async function handler(req,res){
 ...
 try{
  ...
  if(action==='reset'){
   await writeJson(PATHS.searchBuildState,{version:'V8.2',processed:0,total,completed:false,startedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,processed:0,total,completed:false,message:'검색 인덱스 재구축을 시작합니다.'});
  }

  if(action==='step'){
   let st=await readJson(PATHS.searchBuildState,{version:'V8.2',processed:0,total,completed:false});
   if(st.version!=='V8.2'||st.total!==total){st={version:'V8.2',processed:0,total,completed:false,startedAt:new Date().toISOString()};}
   ...
   await writeJson(searchChunkPath(part),{version:'V8.2',part,start,end,records});
  ...
  if(action==='finalize'){
   ...
   await writeJson(PATHS.searchIndex,{version:'V8.2',builtAt:new Date().toISOString(),schoolId:catalog.schoolId||'seongui-high',bookCount:records.length,metadataProcessed:p.processed||0,audit:auditSummary,records});
   const state=await readJson(PATHS.state,{});state.search={ready:true,builtAt:new Date().toISOString(),bookCount:records.length,metadataProcessed:p.processed||0,version:'V8.2',audit:auditSummary};await writeJson(PATHS.state,state);
   await writeJson(PATHS.searchBuildState,{...st,finalized:true,finalizedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,version:'V8.2',bookCount:records.length,audit:auditSummary,message:`검색 인덱스 ${records.length.toLocaleString()}종을 V8.2 기준으로 재구축했습니다.`});
  }
