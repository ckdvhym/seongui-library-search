...
function audit(records){
 const cases=['중국 소설','기후 위기와 관련된 한국 소설','서양 철학','동양 철학','고전 소설','돈 관련 책','재테크 관련 책','연산군','프랑스 전쟁','인생에 대해 돌아볼 수 있는 책','인공지능과 환경 오염','유전공학과 윤리','자유주의와 경제 격차'];
 const out=[];
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(action==='reset'){
   await writeJson(PATHS.searchBuildState,{version:'V7.3',processed:0,total,completed:false,startedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,processed:0,total,completed:false,message:'검색 인덱스 재구축을 시작합니다.'});
  }

  if(action==='step'){
   let st=await readJson(PATHS.searchBuildState,{version:'V7.3',processed:0,total,completed:false});
   if(st.version!=='V7.3'||st.total!==total){st={version:'V7.3',processed:0,total,completed:false,startedAt:new Date().toISOString()};}
   ...
   await writeJson(searchChunkPath(part),{version:'V7.3',part,start,end,records});
  ...
  if(action==='finalize'){
   ...
   await writeJson(PATHS.searchIndex,{version:'V7.3',builtAt:new Date().toISOString(),schoolId:catalog.schoolId||'seongui-high',bookCount:records.length,metadataProcessed:p.processed||0,audit:auditSummary,records});
   const state=await readJson(PATHS.state,{});state.search={ready:true,builtAt:new Date().toISOString(),bookCount:records.length,metadataProcessed:p.processed||0,version:'V7.3',audit:auditSummary};await writeJson(PATHS.state,state);
   await writeJson(PATHS.searchBuildState,{...st,finalized:true,finalizedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,version:'V7.3',bookCount:records.length,audit:auditSummary,message:`검색 인덱스 ${records.length.toLocaleString()}종을 V7.3 기준으로 재구축했습니다.`});
  }
