...
export default async function handler(req,res){
 ...
 try{
  ...
  if(action==='reset'){
   await writeJson(PATHS.searchBuildState,{version:'V7.8',processed:0,total,completed:false,startedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,processed:0,total,completed:false,message:'검색 인덱스 재구축을 시작합니다.'});
  }

  if(action==='step'){
   let st=await readJson(PATHS.searchBuildState,{version:'V7.8',processed:0,total,completed:false});
   if(st.version!=='V7.8'||st.total!==total){st={version:'V7.8',processed:0,total,completed:false,startedAt:new Date().toISOString()};}
   ...
   await writeJson(searchChunkPath(part),{version:'V7.8',part,start,end,records});
  ...
  if(action==='finalize'){
   ...
   await writeJson(PATHS.searchIndex,{version:'V7.8',builtAt:new Date().toISOString(),schoolId:catalog.schoolId||'seongui-high',bookCount:records.length,metadataProcessed:p.processed||0,audit:auditSummary,records});
   const state=await readJson(PATHS.state,{});state.search={ready:true,builtAt:new Date().toISOString(),bookCount:records.length,metadataProcessed:p.processed||0,version:'V7.8',audit:auditSummary};await writeJson(PATHS.state,state);
   await writeJson(PATHS.searchBuildState,{...st,finalized:true,finalizedAt:new Date().toISOString()});
   return res.status(200).json({ok:true,version:'V7.8',bookCount:records.length,audit:auditSummary,message:`검색 인덱스 ${records.length.toLocaleString()}종을 V7.8 기준으로 재구축했습니다.`});
  }
