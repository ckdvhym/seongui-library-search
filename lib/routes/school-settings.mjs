...
const DEFAULTS={
 ...
 event:{enabled:false,title:'',description:'',url:'',startDate:'',endDate:'',imageDataUrl:''},
 logoUpdatedAt:'',
...
function safeSettings(v={}){return {
 ...
 event:{enabled:!!v.event?.enabled,title:String(v.event?.title||'').slice(0,80),description:String(v.event?.description||'').slice(0,220),url:v.event?.url?safeUrl(v.event.url):'',startDate:/^\d{4}-\d{2}-\d{2}$/.test(String(v.event?.startDate||''))?String(v.event.startDate):'',endDate:/^\d{4}-\d{2}-\d{2}$/.test(String(v.event?.endDate||''))?String(v.event.endDate):'',imageDataUrl:/^data:image\/(png|jpeg|webp);base64,/i.test(String(v.event?.imageDataUrl||''))?String(v.event.imageDataUrl):''},
 logoUpdatedAt:String(v.logoUpdatedAt||''),
...
export default async function handler(req,res){
 ...
 try{const body=typeof req.body==='string'?JSON.parse(req.body||'{}'):(req.body||{});const prev=await readJson(PATHS.schoolSettings,DEFAULTS);const logoChanged=Object.prototype.hasOwnProperty.call(body,'logoDataUrl')&&String(body.logoDataUrl||'')!==String(prev?.logoDataUrl||'');const merged={...prev,...body,features:{...(prev?.features||{}),...(body?.features||{})},event:{...(prev?.event||{}),...(body?.event||{})},footerNotice:{...(prev?.footerNotice||{}),...(body?.footerNotice||{})}};if(logoChanged)merged.logoUpdatedAt=new Date().toISOString();const next=safeSettings(merged);if(next.logoDataUrl.length>280000)return res.status(413).json({ok:false,message:'로고 이미지가 너무 큽니다. 200KB 이하의 PNG/JPG/WebP 이미지를 사용해 주세요.'});if(next.event.imageDataUrl.length>1_250_000)return res.status(413).json({ok:false,message:'행사 배너 이미지가 너무 큽니다. 800KB 이하, 권장 1200×400px 이미지를 사용해 주세요.'});await writeJson(PATHS.schoolSettings,next);await addHistory('settings','학교·화면 설정 변경',{features:next.features,accentColor:next.accentColor});if(logoChanged)await addHistory('logo',next.logoDataUrl?'학교 로고 등록·변경':'학교 로고 삭제',{updatedAt:next.logoUpdatedAt});return res.status(200).json({ok:true,message:'학교·화면 설정을 저장했습니다.',...next});}catch(e){return res.status(500).json({ok:false,message:'학교 설정 저장 중 오류가 발생했습니다.',detail:String(e?.message||e)});}
}
