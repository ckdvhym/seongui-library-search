...
export const PATHS={
 ...
 analyticsControl:`${root}/admin/analytics-control.json`,
...
export async function readJson(path, fallback=null, {ttlMs=0,blobCache=false}={}){
  ...
  const task=(async()=>{
    try{const x=await get(path,{access:'private',useCache:!!blobCache});if(!x?.stream)return fallback;const value=JSON.parse(await new Response(x.stream).text());if(ttlMs>0)MEM_CACHE.set(path,{at:Date.now(),value});return value;}
