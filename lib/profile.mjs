...
export function interpretLocal(query){
 ...
 return {query:q,tokens,material:uniq(material),genres:uniq(genres),topics:uniq(topics),emotions:uniq(emotions),origin,classicRequired,classicTarget,classicKorean,focus,wantsAuthor,complex,philosophyTradition,requiredTerms,explicitConcepts,compoundConcepts,softKeywords:[],softTopics:[]};
}
...
function isKoreanClassic(rec){const t=norm(rec.title),x=kdcRaw(rec.kdc),origin=recOriginScore(rec,'한국'),known=CLASSIC_KOREAN_WORKS.some(w=>t.includes(norm(w)));const textual=/한국\s*고전|고전\s*한국|고전소설|고전\s*소설|조선\s*(시대|문학|소설)|고려\s*(시대|문학|소설)/.test(cleanText([rec.title,rec.introPreview,(rec.keywords||[]).join(' ')].join(' ')));return known||(/^81/.test(x)&&classicScore(rec)>=1.2)||(origin>=2&&classicScore(rec)>=1.5)||textual;}
