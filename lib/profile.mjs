...
export function scoreRecord(rec,intent){
 ...
 if(intent.philosophyTradition){score+=14;reasons.push(intent.philosophyTradition);}for(const g of intent.genres){score+=13;reasons.push(g);}for(const m of intent.material){score+=8;reasons.push(m);}for(const e of intent.emotions){score+=10;reasons.push(e);}if(intent.origin){score+=12;reasons.push(`${intent.origin} 문학`);}
