...
export function interpretLocal(query){
 ...
 const abstract=focus.some(f=>f.semantic)||major.career,requiredTerms=concreteTerms(q,tokens,abstract,conceptFocus);
...
export function scoreRecord(rec,intent){
 ...
 const constrained=intent.topics.length||intent.genres.length||intent.emotions.length||intent.origin||intent.classicRequired||intent.focus.length||intent.material.length||intent.philosophyTradition||(intent.requiredTerms||[]).length||(intent.explicitConcepts||[]).length||intent.semanticFallback;
