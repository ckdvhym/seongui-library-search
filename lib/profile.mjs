...
export function interpretLocal(query){
 ...
 const emotions=[];if(/슬픈|슬픔|상실|애도|이별|비극/.test(q))emotions.push('슬픔·상실');if(/위로|회복|치유|힐링|따뜻한|잔잔한|포근한/.test(q))emotions.push('위로·회복');if(/무서운|공포|오싹|섬뜩/.test(q))emotions.push('긴장·공포');if(/유머|웃긴|유쾌|재미있는/.test(q))emotions.push('유머·경쾌');if(/반전/.test(q))emotions.push('반전');
 const focus=[];
