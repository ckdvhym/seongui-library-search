const STOP = new Set('그리고 그러나 하지만 또는 대한 관련 관련된 관한 관한된 위한 통해 있는 없는 하는 되는 되어 된 이 그 저 것 수 등 및 책 도서 내용 이야기 사람 우리 내가 너가 매우 정말 그냥 조금 다시 더 가장 이런 저런 어떤 한 에서 으로 로서 에게 에는 이는 은 는 이 가 을 를 의 과 와 도 에 로 으로부터 대해 돌아볼 돌아보는 연계 연계된 연결 진학 전공 학과 학부 계열 추천 수행 수행평가 평가 발표 토론 보고서 탐구 참고 읽기 읽을 읽으면 좋은 쉽게 설명 설명한 다룬 소재 함께 생각 생각할 생각해볼 미치는 영향 알아볼 학생 희망 원하는 추천할'.split(/\s+/));
...
const TOPICS = [
 ...
 ['의학·약학',['의학','의료','의사','질병','약학','약사','신약','신약개발','의약품','치료','바이오','생명공학','유전','면역','제약','약물','보건','공중보건','보건의료','예방의학','건강정책','역학']],
...
function queryHasTerm(text,term){
 const raw=cleanText(text).toLowerCase(),t=cleanText(term).toLowerCase();if(!raw||!t)return false;
 const nt=norm(t);
 if(/^[가-힣]{1,2}$/.test(t)){
  if(semanticPhraseCount(raw,t)>0)return true;
  const shortRx=new RegExp(`(^|[^가-힣a-z0-9])${escRx(t)}(?:책|도서|관련|분야|주제|학과|학부|전공)(?=$|[^가-힣a-z0-9])`,'g');
  return shortRx.test(raw);
 }
 if(/^[가-힣\s]+$/.test(t)&&nt.length>=3)return norm(raw).includes(nt);
 return wholePhraseCount(raw,t)>0 || (nt.length>=3&&norm(raw).includes(nt));
}
function queryMatchesAny(text,terms){return (terms||[]).some(t=>queryHasTerm(text,t));}
...
function queryAnchorTerms(q){
 const found=[];
 for(const [,terms] of TOPICS){for(const t of terms){if(norm(t).length>=2&&queryHasTerm(q,t))found.push(t);}}
 found.sort((a,b)=>norm(b).length-norm(a).length);
 const out=[];
 for(const t of found){const nt=norm(t);if(out.some(x=>norm(x).includes(nt)))continue;out.push(t);if(out.length>=4)break;}
 return out;
}

const MATERIAL_ANCHORS=new Set(['소설','문학','시','시집','에세이','수필','독서']);
function primaryAnchorTerms(q,material=[]){
 return queryAnchorTerms(q).filter(t=>!MATERIAL_ANCHORS.has(cleanText(t))&&!isPurposeNoiseToken(t)&&!material.some(m=>norm(m)===norm(t))).slice(0,3);
}
function purposeContext(q){
 return /수행\s*평가|평가|발표|토론|보고서|탐구|참고|읽을|읽으면|읽기|추천|관심|진로|진학|희망|학생|알아보|찾고|찾아|공부|과제/.test(q);
}
function topicForAnchor(anchor){
 const na=norm(anchor);for(const [name,terms] of TOPICS){if(terms.some(t=>norm(t)===na))return name;}return null;
}
function buildConceptGroups(q,{material=[],requiredTerms=[],major=null,explicitConcepts=[]}={}){
 const out=[];
 const add=(name,direct,topic=null,source='query')=>{const key=norm(name);if(!key||out.some(x=>norm(x.name)===key))return;out.push({name, direct:uniq(direct||[name]), topic, source});};
 for(const a of primaryAnchorTerms(q,material)){const ex=(explicitConcepts||[]).find(c=>norm(c.name)===norm(a)||(c.terms||[]).some(t=>norm(t)===norm(a)));add(a,ex?.terms?.length?ex.terms:[a],topicForAnchor(a),'anchor');}
 for(const t of requiredTerms||[])add(t,[t],topicForAnchor(t),'required');
 if(major?.area && !out.length)add(major.area.name,major.area.core||major.area.terms,major.area.topics?.[0]||null,'major');
 // '도시 재생', '문화재 보존'처럼 붙어서 하나의 전문 개념을 이루는 두 핵심어는
 // 각 단어를 따로 완화하지 않고 하나의 구문으로 먼저 해석한다. 이렇게 하면 마지막 완화 단계에서
 // '도시'만 맞는 책, '보존'만 맞는 책이 섞이는 것을 줄이고 Gemini 의미확장도 구문 전체를 기준으로 작동한다.
 if(out.length===2){const a=cleanText(out[0].name),b=cleanText(out[1].name),raw=cleanText(q);const adjacent=new RegExp(`${escRx(a)}\\s+${escRx(b)}`).test(raw)||new RegExp(`${escRx(b)}\\s+${escRx(a)}`).test(raw);if(adjacent){const topic=out.find(x=>x.topic)?.topic||null;return [{name:`${a} ${b}`,direct:[`${a} ${b}`,`${a}${b}`],topic,source:'phrase'}];}}
 return out.slice(0,4);
}
function knownNormTerms(){return new Set([...TOPICS.flatMap(x=>x[1]),...GENRES.flatMap(x=>x[1]),...EMOTIONS.flatMap(x=>x[1]),...EXPLICIT_CONCEPTS.flatMap(x=>x.terms),...Object.values(ORIGIN_STRONG).flat(),'관련 관한 주제 문제 대해 배경 소설 문학 시집 에세이 수필 고전 서양 동양 철학 책 도서 한국 국내 우리나라 중국 일본 프랑스 독일 영미 영국 미국 이탈리아 스페인 포르투갈'.split(/\s+/)].map(norm));}
...
const PURPOSE_NOISE_RX=/^(?:수행|수행평가|평가|발표|토론|토론할|보고서|탐구|참고|참고할|읽기|읽을|읽으면|읽어볼|좋은|좋을|쉽게|설명|설명한|다룬|다루는|소재|소재로|함께|생각|생각할|생각해볼|알아볼|미치는|영향|영향을|영향에|학생|학생이|학생에게|학생들이|희망|희망하는|원하는|되고|싶은|일하고|쓸|쓸만한|어렵지|않은|추천할|추천해줄|진로인|관심|관심있는|관심이|힘들)$/;
function isPurposeNoiseToken(token){
 const t=cleanText(token);if(!t)return true;
 return PURPOSE_NOISE_RX.test(t)||/^(?:관심|추천|참고|읽|학생|진로|진학|전공|학과|학부|희망|수행|평가|발표|토론|보고서|탐구|좋|알아|찾|필요|공부|과제|있|되|싶|원하|관련|관한|대해)/.test(t);
}
function stripGenericJosaToken(token){
 const raw=String(token||'').trim();if(!raw)return raw;
 const suffixes=['으로부터','에서부터','에게서','한테서','으로','에게','한테','에서','까지','부터','처럼','보다','은','는','을','를','에','의','도','만','과','와','랑','이랑','로'];
 for(const suf of suffixes){if(raw.endsWith(suf)){const base=raw.slice(0,-suf.length);if(base.length>=2)return base;}}
 for(const suf of ['이','가']){if(raw.endsWith(suf)){const base=raw.slice(0,-1);if(base.length>=3)return base;}}
 return raw;
}
function careerCoreTerms(q){
 const text=cleanText(q);
 // 학과명은 개별 목록으로 하드코딩하지 않고 전공명에서 핵심 어근만 추출한다.
 const m=text.match(/([가-힣A-Za-z0-9·+&\- ]{2,30}?)(?:과|학부|전공)(?:은|는|이|가|을|를|에|에서|으로|로)?(?=\s|$)/);
 if(!m)return[];
 const phrase=m[1].replace(/(?:진로|희망|관심|관련|추천)\s*$/,'').trim();
 const raw=uniq(tokenize(phrase).map(stripGenericJosaToken).filter(t=>t.length>=2&&!isPurposeNoiseToken(t)&&!KNOWN.has(norm(t))));
 const out=[];
 for(const t of raw){
  if(t.endsWith('공학')&&t.length>2)out.push(t.slice(0,-2));
  else if(t.endsWith('학')&&t.length>=3)out.push(t.slice(0,-1));
  else out.push(t);
 }
 return uniq(out).slice(-3);
}

function concreteTerms(q,tokens,hasAbstractFocus,focus=[],extraTerms=[]){
 const focusNorm=new Set(focus.flatMap(f=>f.terms||[]).map(norm));const out=[...(extraTerms||[])];
 if(hasAbstractFocus&&!(extraTerms||[]).length)return[];
 for(const raw of tokens){
  if(/(?:학과|학부|전공)(?:은|는|이|가|을|를|에|에서|으로|로)?$/.test(raw))continue;
  const t=stripGenericJosaToken(stripKnownJosaToken(raw)),nt=norm(t);if(!t||isPurposeNoiseToken(t))continue;
  if(/(?:학과|학부|전공)$/.test(t))continue;
  if(t==='청소년'&&/청소년(?:이|가)?\s*(?:읽|보기)/.test(q))continue;
  if(nt.endsWith('책')){const base=nt.slice(0,-1);if(KNOWN.has(base))continue;}
  if(KNOWN.has(nt)||focusNorm.has(nt))continue;
  if([...(focus||[])].some(f=>(f.terms||[]).some(x=>norm(x).includes(nt)||nt.includes(norm(x)))))continue;
  if(/^(관련|관한|대한|위한|추천|진로|관심|연결|내용|이야기|분야|배경|고전|한국|국내|우리나라|일본|중국|프랑스|독일|영미|영국|미국|이탈리아|스페인|포르투갈|문학|소설|시|시집|에세이|수필)$/.test(t))continue;
...
export function interpretLocal(query){
 ...
 const genres=[];for(const [name,terms] of GENRES)if(queryMatchesAny(q,terms))genres.push(name);
 let topics=[];for(const [name,terms] of TOPICS)if(queryMatchesAny(q,terms))topics.push(name);if(material.length&&!/문학|글쓰기|문장|작가/.test(q))topics=topics.filter(t=>t!=='문학·글쓰기');
 ...
 if(/단편\s*소설|단편집|단편선|소설집|중단편/.test(q))focus.push({name:'단편소설',kind:'short-story',terms:['단편소설','단편 소설','단편집','소설집','단편선','중단편','단편 작품','단편들을'],strict:true});
 ...
 // '일본문학'과 '일본 문학'처럼 띄어쓰기만 다른 문학권 질의는 같은 의도로 처리한다.
 if(origin&&/문학/.test(q))topics=topics.filter(t=>t!=='문학·글쓰기');
 ...
 const compoundTopics=topics.length>=2 && hasCompoundConnector(q);
 ...
 const conceptFocus=[...focus,...explicitConcepts.map(c=>({terms:c.terms||[]}))];
 const careerCore=major.career&&!major.area?careerCoreTerms(q):[];
 const abstract=focus.some(f=>f.semantic)||major.career,requiredTerms=concreteTerms(q,tokens,abstract,conceptFocus,careerCore);
 const complex=/진로|진학|학과|학부|전공|연계|수행\s*평가|평가|발표|토론|보고서|탐구|참고|읽을|읽으면|관심|연결|하지만|싫어|재미있|추천|관련|한국|국내|중국|일본|프랑스|영미|고전|재테크|인생|삶|철학|힐링|학교\s*배경|학원물/.test(q)||tokens.length>=5;
 ...
 const anchors=primaryAnchorTerms(q,material),conceptGroups=buildConceptGroups(q,{material,requiredTerms,major,explicitConcepts});
 const phraseGroup=conceptGroups.length===1&&conceptGroups[0]?.source==='phrase';
 const effectiveRequiredTerms=phraseGroup?[]:requiredTerms;
 return {query:q,tokens,anchorTerms:anchors,conceptGroups,purposeContext:purposeContext(q),compoundTopics,majorIntent:major.career,majorArea:major.area?.name||null,material:uniq(material),genres:uniq(genres),topics:uniq(topics),emotions:uniq(emotions),origin,classicRequired,classicTarget,classicKorean,focus,wantsAuthor,complex,directLookup,philosophyTradition,requiredTerms:effectiveRequiredTerms,explicitConcepts,compoundConcepts,softKeywords:[],softTopics:[]};
}
...
function originPass(rec,name){
 const mt=rec.materialType||((rec.materialTypes||[])[0])||'문학', k=originFromKdc(rec.kdc,mt);
 // KDC가 특정 문학권을 명확히 가리키면 소개글의 우연한 국가 언급보다 KDC를 우선한다.
 if(k)return k===name;
 return isLiterary(rec)&&recOriginScore(rec,name)>=4;
}
function semanticPhraseCount(text,term){
 const raw=cleanText(text).toLowerCase(),t=cleanText(term).toLowerCase();if(!raw||!t)return 0;
 const particles='(?:은|는|이|가|을|를|과|와|랑|이랑|의|에|에서|로|으로|도|만)?';
 const rx=new RegExp(`(^|[^가-힣a-z0-9])${escRx(t)}${particles}(?=$|[^가-힣a-z0-9])`,'g');
 return (raw.match(rx)||[]).length;
}
function strictMaterialMatch(rec,m){
 const k=kdcRaw(rec.kdc), title=cleanText(rec.title), intro=cleanText(rec.introPreview), text=[title,intro].join(' ');
 if(m==='소설'){
  if(/^8\d3/.test(k)||/^808\.3/.test(k))return true;
  // 세계문학전집 808 분류 안의 고전 작품은 작품명이 명확한 경우 소설로 인정한다.
  if(/^808/.test(k)&&CLASSIC_KOREAN_WORKS.some(w=>norm(title).includes(norm(w))))return true;
  if(/장편\s*소설|단편\s*소설|소설집|중단편\s*소설|novel/i.test(text))return true;
  return false;
 }
 if(m==='시'){
  if(/^8\d1/.test(k)||/^808\.1/.test(k))return true;
  if(/시집|시선집|시\s*모음|시\s*100선|시\s*백선|poetry\s*collection/i.test(text))return true;
  return false;
 }
 if(m==='에세이'){
  if(/^8\d4/.test(k)||/^818/.test(k))return true;
  if(/에세이|수필집|산문집/i.test(text))return true;
  return false;
 }
 return (rec.materialTypes||[rec.materialType]).includes(m)||(rec.genres||[]).includes(m);
}
function materialMatch(rec,m,{strict=false}={}){return strict?strictMaterialMatch(rec,m):((rec.materialTypes||[rec.materialType]).includes(m)||(rec.genres||[]).includes(m));}
...
function focusMatch(rec,f){
 ...
 if(f.kind==='school-setting'){
  ...
  const instructional=recTextEvidence(rec,['교과서','소설 읽기','수업','교재','작품 해설','문제집','국어교사','교사모임','선생님들이','함께 읽고','대화한 내용']);
  const storySignal=recTextEvidence(rec,['주인공','전학생','친구','동급생','교실','학교생활','학창시절','등교','사건','이야기']);
  // 학교를 소재로 '가르치는 책'과 실제 학교를 무대로 전개되는 이야기를 구분한다.
  if((instructional.titleHits>0||instructional.introHits>=2) && storySignal.titleHits===0 && storySignal.introHits<2)return {ok:false,ev};
  ...
  return {ok,ev:{...ev,score:ev.score+place.score*.8+storySignal.score*.25}};
 }
 if(f.kind==='short-story'){
  const direct=recTextEvidence(rec,['단편소설','단편 소설','단편집','소설집','단편선','중단편','단편 작품','단편들을']);
  const titleDirect=['단편','단편선','단편집','소설집','중단편'].some(t=>norm(rec.title).includes(norm(t)));
  const ok=titleDirect||direct.titleHits>0||direct.introHits>0||(direct.distinct>=2&&direct.score>=4);
  return {ok,ev:direct};
 }
 if(f.major){
...
function semanticExpansionMatch(rec,intent){
 const direct=uniq(intent.semanticDirectKeywords||[]),related=uniq(intent.semanticRelatedKeywords||[]),context=uniq(intent.semanticContextKeywords||[]);
 // 이전 응답 형식과의 호환: direct/related가 없을 때만 core를 related로 사용한다.
 const legacy=uniq(intent.semanticCoreKeywords||[]);
 const d=direct.length?direct:[cleanText(intent.query)];
 const r=related.length?related:legacy.filter(x=>!d.includes(x));
 const dev=recTextEvidence(rec,d,{history:isHistoryQuery(intent)}),rev=r.length?recTextEvidence(rec,r,{history:isHistoryQuery(intent)}):{score:0,distinct:0,titleHits:0,introHits:0,tocHits:0},xev=context.length?recTextEvidence(rec,context,{history:isHistoryQuery(intent)}):{score:0,distinct:0,titleHits:0,introHits:0,tocHits:0};
 const directTitle=d.reduce((n,t)=>n+semanticPhraseCount(rec.title,t),0),directIntro=d.reduce((n,t)=>n+semanticPhraseCount(rec.introPreview,t),0);
 const relatedTitle=r.reduce((n,t)=>n+semanticPhraseCount(rec.title,t),0),relatedIntro=r.reduce((n,t)=>n+semanticPhraseCount(rec.introPreview,t),0);
 // 통과 기준: 직접 동의어가 실제 문장에 있거나, 강한 연관개념이 제목에 있거나,
 // 서로 다른 강한 연관개념이 2개 이상 확인될 때. 넓은 context 단어 하나만으로는 절대 통과시키지 않는다.
 let ok=directTitle>0||directIntro>0||relatedTitle>0||(rev.distinct>=2&&(relatedIntro>=2||rev.tocHits>=2))||(relatedIntro>=1&&rev.distinct>=1&&xev.distinct>=2);
 if(intent.semanticKind==='genre'){
   const childKdc=/^8(?:0[89]|1[0138]|2[0138]|3[0138]|4[0138]|5[0138]|6[0138]|7[0138]|8[0138]|9[0138])/.test(kdcRaw(rec.kdc));
   ok=ok||(childKdc&&(directIntro>0||relatedTitle>0||(relatedIntro>=1&&rev.distinct>=2)));
 }
 const score=Math.min(19,8+dev.score*.6+rev.score*.45+xev.score*.08+(directTitle+relatedTitle)*2);
 return {ok,score,ev:dev,related:rev,context:xev,directTitle,directIntro,relatedTitle,relatedIntro};
}
...
function explicitGenreMatch(rec,g){
 ...
 if(g==='로맨스'){
  ...
  const otherDominant=recTextEvidence(rec,['무협','무림','강호','무공','검객','협객','사조삼부곡','무림지존','도룡도','의천검','대하소설','전쟁','전투']);
  const explicit=direct.titleHits>0||direct.introHits>0;
  // '로맨스 소설'에서는 사랑 요소가 곁가지인 무협/전쟁 중심 작품을 장르 오인으로 제외한다.
  // 저장된 로맨스 태그나 소개 속 한두 번의 '사랑' 표현보다 작품의 주 장르를 우선한다.
  if(otherDominant.titleHits>0||otherDominant.introHits>=1)return false;
...
function conceptEvidence(rec,group,intent){
 const direct=recTextEvidence(rec,group.direct||[group.name],{history:isHistoryQuery(intent)});
 const topicEv=group.topic?recTextEvidence(rec,topicTerms(group.topic),{history:group.topic==='역사'}):{score:0,distinct:0,titleHits:0,introHits:0,keywordHits:0,tocHits:0};
 const primaryTopic=group.topic&&(rec.primaryTopics||[]).includes(group.topic);
 const secondaryTopic=group.topic&&(rec.topics||rec.secondaryTopics||[]).includes(group.topic);
 return {direct,topicEv,primaryTopic,secondaryTopic};
}
function conceptPass(rec,intent,relax){
 const groups=intent.conceptGroups||[];if(!groups.length)return {ok:true,score:0,reasons:[],matched:0,total:0};
 let matched=0,strong=0,score=0;const reasons=[];
 for(const g of groups){
  const e=conceptEvidence(rec,g,intent),d=e.direct,te=e.topicEv;
  const isStrong=d.titleHits>0||d.introHits>0||d.keywordHits>=2;
  const topicStrong=!!g.topic&&(te.titleHits>0||te.introHits>=2||te.distinct>=2||te.score>=7);
  const isMedium=isStrong||d.keywordHits>0||d.tocHits>=2||topicStrong;
  const isWeak=isMedium||e.primaryTopic||e.secondaryTopic||(!!g.topic&&te.score>0);
  let ok=relax===0?isStrong:(relax===1?isMedium:isWeak);
  if(!ok && groups.length===1 && intent.semanticAssist){
   const sm=semanticExpansionMatch(rec,intent);
   ok=relax===1?(sm.relatedTitle>0||sm.relatedIntro>0||sm.related.distinct>=2):sm.ok;
   if(ok)score+=Math.min(9,sm.score*.5);
  }
  if(ok){matched++;if(isStrong)strong++;score+=isStrong?(12+Math.min(8,d.score*.35)):(isMedium?(7+Math.min(4,d.score*.2)):3);reasons.push(g.name);}
 }
 if(groups.length===1)return {ok:matched===1,score,reasons,matched,total:1};
 // 복합 주제는 0~2단계까지 모든 핵심 개념의 근거를 유지한다.
 // 한 개념만 맞는 넓은 결과는 마지막 안전망(3단계)에서만 허용해 관련성 퇴행을 막는다.
 if(relax<=2)return {ok:matched===groups.length,score,reasons,matched,total:groups.length};
 return {ok:matched>=1&&strong>=1,score:score-(groups.length-matched)*7,reasons,matched,total:groups.length};
}

export function scoreRecord(rec,intent){
 const relax=Math.max(0,Number(intent.relaxLevel||0));
 const qn=norm(intent.query),titleN=norm(rec.title),authorN=norm(rec.author);let score=0;const reasons=[];
 if(titleN&&qn===titleN){score+=22;reasons.push('제목 일치');}
 else if(intent.directLookup&&qn.length>=2&&titleN.includes(qn)){score+=14;reasons.push('제목 부분 일치');}
 if(authorN&&qn.includes(authorN)){score+=intent.wantsAuthor?18:4;reasons.push('저자 일치');}
 // 자연어 문장의 핵심 개념을 먼저 검증한다. '관심 있음/수행평가/추천' 같은 목적어는 관련성 근거가 아니다.
 const cp=conceptPass(rec,intent,relax);if(!cp.ok)return null;score+=cp.score;reasons.push(...cp.reasons);

 if(intent.material.length&&!intent.material.every(m=>materialMatch(rec,m,{strict:true})))return null;
 if(intent.classicKorean){if(!isKoreanClassic(rec))return null;}else if(intent.origin&&!originPass(rec,intent.origin))return null;
 if(intent.genres.length&&!intent.genres.every(g=>explicitGenreMatch(rec,g)))return null;
 if(intent.emotions.length&&!intent.emotions.every(e=>emotionMatch(rec,e))){
  if(relax===0)return null;
  const ev=recTextEvidence(rec,['슬픔','상실','이별','눈물','애도','그리움','외로움','아픔','위로','회복','치유','희망','따뜻','포근','잔잔']);
  if(relax===1&&!(ev.titleHits>0||ev.introHits>0||ev.distinct>=2))return null;
  if(relax>=2&&ev.score<=0)return null;
  score+=Math.min(7,2+ev.score*.35);reasons.push('감정 연관');
 }
 if(intent.philosophyTradition&&!philosophyMatch(rec,intent.philosophyTradition))return null;
 if(intent.classicRequired){if(intent.classicTarget==='소설'&&!materialMatch(rec,'소설',{strict:true}))return null;if(intent.classicTarget==='문학'&&!isLiterary(rec))return null;if(!intent.classicTarget&&!isLiterary(rec))return null;if(rec.classicMeta)return null;const minClassic=intent.classicKorean?0.8:((intent.origin==='한국'&&intent.classicTarget==='문학')?1.5:2.2);if(classicScore(rec)<minClassic&&!intent.classicKorean)return null;if(intent.classicKorean&&!isKoreanClassic(rec))return null;score+=13+Math.min(7,Math.max(classicScore(rec),intent.classicKorean?2:0));reasons.push(intent.classicTarget==='소설'?'고전 소설 작품':(intent.classicKorean?'한국 고전 문학 작품':'고전 문학 작품'));}

 // 'A와 B', 'A과 B'처럼 두 개념을 직접 연결한 검색은 둘 다 실제 근거가 있어야 한다.
 for(const c of intent.explicitConcepts||[]){const m=explicitConceptMatch(rec,c);const hard=c.required;const compoundHard=intent.compoundConcepts&&relax<=1;if((hard||compoundHard)&&!m.ok)return null;if(m.ok){score+=Math.min(14,5+m.ev.score);reasons.push(c.name);}else if(intent.compoundConcepts&&relax===2){score-=4;}}

 // 로컬 규칙이 모르는 짧은 개념은 Gemini가 '책 추천'이 아니라 의미의 핵심어만 확장한다.
 // 이 층은 기존에 잘 작동하는 검색에는 켜지지 않는다.
 if(intent.semanticFallback){const sm=semanticExpansionMatch(rec,intent);if(!sm.ok)return null;score+=sm.score;reasons.push('의미 연관');}

 // 사용자가 직접 쓴 구체명(연산군, 프랑스 등)은 제목·소개·목차 중 실제 근거가 있어야 한다.
 const hist=isHistoryQuery(intent);
 for(const term of intent.requiredTerms||[]){
  const ev=recTextEvidence(rec,[term],{history:hist});
  const strong=hist?(ev.titleHits+ev.introHits+ev.tocHits>0):(ev.titleHits+ev.introHits>0||ev.keywordHits>=2);
  const medium=strong||ev.keywordHits>0||ev.tocHits>=2;
  let ok=relax===0?strong:(relax===1?medium:false);
  if(!ok&&relax>=1&&intent.semanticAssist){
    const sm=semanticExpansionMatch(rec,intent);
    ok=relax===1?(sm.relatedTitle>0||sm.relatedIntro>=1||sm.related.distinct>=2):sm.ok;
    if(ok){score+=Math.min(10,sm.score*.55);reasons.push('의미 확장');}
  }
  if(!ok&&relax>=2){
    // 마지막 단계에서도 자료형/장르는 유지하고, 구체어는 넓은 의미근거가 있을 때만 완화한다.
    const sm=intent.semanticAssist?semanticExpansionMatch(rec,intent):null;
    ok=!!(sm&&sm.ok);
  }
  if(!ok)return null;
  if(strong){score+=Math.min(13,4+ev.score);reasons.push(term);}
  else if(medium){score+=Math.min(7,2+ev.score*.5);reasons.push(term+' 관련');}
 }

 for(const t of intent.topics){const ev=recTextEvidence(rec,topicTerms(t),{history:t==='역사'});const strict=!intent.conceptGroups?.length&&(!intent.majorIntent&&/관련|관한|주제|문제|대해/.test(intent.query));if(strict){const ok=relax===0?(ev.titleHits>0||ev.introHits>=2||ev.distinct>=2||ev.score>=7):(relax===1?(ev.titleHits>0||ev.introHits>0||ev.distinct>=2||ev.score>=4):(ev.score>0));if(!ok)return null;}if(ev.titleHits>0){score+=13;reasons.push(t);}else if(ev.score>=7){score+=9;reasons.push(t);}else if(ev.score>=3){score+=3;}}
 for(const f of intent.focus||[]){const m=focusMatch(rec,f);if(!m.ok){if(!(relax>0&&f.semantic))return null;const ev2=recTextEvidence(rec,f.terms||[]);if(relax===1&&!(ev2.titleHits>0||ev2.introHits>0||ev2.distinct>=2))return null;if(relax>=2&&ev2.score<=0)return null;score+=Math.min(8,2+ev2.score*.35);reasons.push(f.name+' 관련');continue;}const ev=m.ev||{score:0};if(f.major)score+=Math.min(20,8+ev.score);else if(f.semantic)score+=Math.min(17,5+ev.score);else score+=Math.min(15,5+ev.score);reasons.push(f.name);}
 if(intent.philosophyTradition){score+=14;reasons.push(intent.philosophyTradition);}for(const g of intent.genres){score+=13;reasons.push(g);}for(const m of intent.material){score+=8;reasons.push(m);}for(const e of intent.emotions){score+=13;reasons.push(e);}if(intent.origin){score+=12;reasons.push(`${intent.origin} 문학`);}
 ...
 const searchN=ntext(rec.title,rec.author,rec.publisher,(rec.keywords||[]).join(' '),(rec.tocTokens||[]).join(' '));let tokenMatches=0;for(const t of intent.tokens){
  // '일본문학'과 '일본 문학'처럼 띄어쓰기만 다른 문학권 검색의 순위를 동일하게 유지한다.
  if(intent.origin&&/(한국|국내|우리나라|일본|중국|프랑스|독일|영미|영국|미국|이탈리아|스페인|포르투갈|문학|소설|시집|에세이|수필)/.test(t))continue;
  if(searchN.includes(norm(t))){score+=.35;tokenMatches++;}
 }
 const constrained=intent.topics.length||intent.genres.length||intent.emotions.length||intent.origin||intent.classicRequired||intent.focus.length||intent.material.length||intent.philosophyTradition||(intent.requiredTerms||[]).length||(intent.explicitConcepts||[]).length||(intent.conceptGroups||[]).length||intent.semanticFallback;
 const min=constrained?(relax===0?12:(relax===1?9:7)):Math.max(5,Math.min(10,2+intent.tokens.length));if(score<min)return null;if(!constrained&&!titleN.includes(qn)&&!authorN.includes(qn)&&tokenMatches===0)return null;
 return {score,reasons:uniq(reasons).slice(0,4)};
}

export function auditRecordAgainstIntent(rec,intent){
 const problems=[];if(intent.material.length&&!intent.material.every(m=>materialMatch(rec,m,{strict:true})))problems.push('자료형');if(intent.classicKorean&&!isKoreanClassic(rec))problems.push('한국고전근거');else if(intent.origin&&!originPass(rec,intent.origin))problems.push('문학권');if(intent.philosophyTradition&&!philosophyMatch(rec,intent.philosophyTradition))problems.push('철학권역');if(intent.classicRequired){const minClassic=intent.classicKorean?0.8:((intent.origin==='한국'&&intent.classicTarget==='문학')?1.5:2.2);if(classicScore(rec)<minClassic&&!intent.classicKorean)problems.push('고전근거');if(intent.classicTarget==='소설'&&!materialMatch(rec,'소설',{strict:true}))problems.push('고전자료형');if(intent.classicTarget==='문학'&&!isLiterary(rec))problems.push('고전자료형');}for(const t of intent.requiredTerms||[]){const ev=recTextEvidence(rec,[t],{history:isHistoryQuery(intent)});if(ev.titleHits+ev.introHits+ev.tocHits===0)problems.push(`필수어:${t}`);}for(const c of intent.explicitConcepts||[]){if((intent.compoundConcepts||c.required)&&!explicitConceptMatch(rec,c).ok)problems.push(`${c.required?'필수개념':'복합개념'}:${c.name}`);}if(intent.semanticFallback&&!semanticExpansionMatch(rec,intent).ok)problems.push('AI의미근거');return problems;
}
