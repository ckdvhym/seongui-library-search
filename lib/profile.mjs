...
const MAJOR_AREAS = [
 {name:'경제학', rx:/경제\s*(학과|학부|전공)|경제학/, topics:['경제·경영'], terms:['경제','경제학','시장','금융','소비','소득','금리','거시경제','미시경제']},
 {name:'간호학', rx:/간호\s*(학과|학부|전공)|간호학/, topics:['의학·약학'], terms:['간호','간호학','간호사','환자','임상','의료','보건']},
 {name:'컴퓨터과학', rx:/(컴퓨터|소프트웨어|정보)\s*(학과|학부|전공)|컴퓨터공학|컴퓨터과학|소프트웨어학/, topics:['인공지능·정보'], terms:['컴퓨터','컴퓨터과학','프로그래밍','코딩','소프트웨어','알고리즘','데이터','인공지능','정보기술']},
 {name:'생명공학', rx:/생명\s*공학\s*(과|학과|학부|전공)?|바이오\s*(공학|학과|전공)/, topics:['의학·약학','과학'], terms:['생명공학','바이오','유전공학','유전자','유전체','분자생물학','세포','생명과학','생명기술']},
 {name:'수의학', rx:/수의\s*(학과|학부|전공)|수의학/, topics:['의학·약학'], terms:['수의학','수의사','동물의료','동물병원','동물질병','가축질병']},
 {name:'약학', rx:/약\s*(학과|학부|전공)|약학/, topics:['의학·약학'], terms:['약학','약사','의약품','약물','제약','신약']},
 {name:'건축학', rx:/건축\s*(학과|학부|전공)|건축학/, topics:['건축·도시'], terms:['건축','건축학','건축가','도시','공간','주거','건물']},
 {name:'심리학', rx:/심리\s*(학과|학부|전공)|심리학/, topics:['심리·감정'], terms:['심리','심리학','행동','인지','감정','상담','관계']}
];
...
export function interpretLocal(query){
 ...
 if(major.area){for(const t of major.area.topics)if(!topics.includes(t))topics.push(t);focus.push({name:major.area.name+' 전공',terms:major.area.terms,strict:false,semantic:true});}
 const abstract=focus.some(f=>f.semantic)||major.career,requiredTerms=concreteTerms(q,tokens,abstract,focus);
 const complex=/진로|진학|학과|학부|전공|연계|수행평가|탐구|관심|연결|하지만|싫어|재미있|추천|관련|한국|국내|중국|일본|프랑스|영미|고전|재테크|인생|삶|철학/.test(q)||tokens.length>=5;
