...
function semanticExpansionMatch(rec,intent){
 const core=uniq(intent.semanticCoreKeywords||[]),context=uniq(intent.semanticContextKeywords||[]);
 if(!core.length)return {ok:false,score:0};
 const cev=recTextEvidence(rec,core,{history:isHistoryQuery(intent)}),xev=context.length?recTextEvidence(rec,context,{history:isHistoryQuery(intent)}):{score:0,distinct:0,titleHits:0,introHits:0,tocHits:0};
 const titlePhraseHits=core.reduce((n,t)=>n+semanticPhraseCount(rec.title,t),0);
 // AI 확장은 '가까운 개념'만 후보 근거로 쓴다. 단어 하나가 소개에 우연히 등장한 것만으로는 통과시키지 않는다.
 let ok=titlePhraseHits>0||cev.introHits>=2||cev.distinct>=2||cev.tocHits>=2||(cev.introHits>=1&&xev.distinct>=1);
 if(intent.semanticKind==='genre'){
   ...
   ok=titlePhraseHits>0||cev.introHits>=2||cev.distinct>=2||(cev.introHits>=1&&xev.distinct>=1)||(childKdc&&cev.introHits>=1);
 }
 return {ok,score:Math.min(18,8+cev.score*.55+xev.score*.18),ev:cev,context:xev,titlePhraseHits};
}
