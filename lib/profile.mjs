...
const EXPLICIT_CONCEPTS = [
 ...
 {name:'수의사', query:[/수의사/,/수의학/], terms:['수의사','수의학','동물병원','동물 의료','동물의료','동물 진료','동물진료','반려동물 진료','가축 질병','동물 질병'], required:true},
...
function extractExplicitConcepts(q){
 const out=[];
 for(const c of EXPLICIT_CONCEPTS){if(c.query.some(rx=>rx.test(q)))out.push({name:c.name,terms:c.terms,required:!!c.required});}
 return out;
}
...
export function auditRecordAgainstIntent(rec,intent){
 const problems=[];if(intent.material.length&&!intent.material.every(m=>materialMatch(rec,m)))problems.push('자료형');if(intent.origin&&recOriginScore(rec,intent.origin)<3)problems.push('문학권');if(intent.philosophyTradition&&!philosophyMatch(rec,intent.philosophyTradition))problems.push('철학권역');if(intent.classicRequired&&classicScore(rec)<2.2)problems.push('고전근거');for(const t of intent.requiredTerms||[]){const ev=recTextEvidence(rec,[t],{history:isHistoryQuery(intent)});if(ev.titleHits+ev.introHits+ev.tocHits===0)problems.push(`필수어:${t}`);}for(const c of intent.explicitConcepts||[]){if((intent.compoundConcepts||c.required)&&!explicitConceptMatch(rec,c).ok)problems.push(`${c.required?'필수개념':'복합개념'}:${c.name}`);}return problems;
}
