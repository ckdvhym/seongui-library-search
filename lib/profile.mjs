...
function conceptPass(rec,intent,relax){
 ...
 for(const g of groups){
  ...
  if(ok){matched++;if(isStrong)strong++;score+=isStrong?12:(isMedium?7:3);reasons.push(g.name);}
 }
 if(groups.length===1)return {ok:matched===1,score,reasons,matched,total:1};
 // 복합 주제: 정확 단계에서는 모든 개념을 지키고, 마지막 관련자료 단계에서만 한 개념 누락을 허용한다.
 if(relax<=1)return {ok:matched===groups.length,score,reasons,matched,total:groups.length};
 if(relax===2)return {ok:matched===groups.length || (matched>=groups.length-1&&strong>=1),score:score-(matched<groups.length?5:0),reasons,matched,total:groups.length};
