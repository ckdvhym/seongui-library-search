...
const EXPLICIT_CONCEPTS = [
 ...
 {name:'환경 오염', query:[/환경\s*오염/,/오염\s*문제/], terms:['환경오염','환경 오염','대기오염','수질오염','토양오염','해양오염','플라스틱 오염','미세플라스틱','오염물질','폐기물 오염']},
...
function extractExplicitConcepts(q){
 const out=[];
 for(const c of EXPLICIT_CONCEPTS){if(c.query.some(rx=>rx.test(q)))out.push({name:c.name,terms:c.terms});}
 return out;
}
...
function knownNormTerms(){return new Set([...TOPICS.flatMap(x=>x[1]),...GENRES.flatMap(x=>x[1]),...EMOTIONS.flatMap(x=>x[1]),...EXPLICIT_CONCEPTS.flatMap(x=>x.terms),'관련 관한 주제 문제 대해 소설 문학 시집 에세이 수필 고전 서양 동양 철학 책 도서 한국 국내 우리나라 중국 일본 프랑스 독일 영미 영국 미국 이탈리아 스페인 포르투갈'.split(/\s+/)].map(norm));}
const KNOWN=knownNormTerms();
function concreteTerms(q,tokens,hasAbstractFocus,focus=[]){if(hasAbstractFocus)return[];const focusNorm=new Set(focus.flatMap(f=>f.terms||[]).map(norm));const out=[];for(const t of tokens){const nt=norm(t);if(KNOWN.has(nt)||focusNorm.has(nt))continue;if([...(focus||[])].some(f=>(f.terms||[]).some(x=>norm(x).includes(nt)||nt.includes(norm(x)))))continue;if(/^(관련|관한|대한|위한|추천|진로|관심|연결|내용|이야기|분야)$/.test(t))continue;if(t.length>=2)out.push(t);}return uniq(out).slice(0,4);}
...
export function interpretLocal(query){
 ...
 const abstract=focus.some(f=>f.semantic),requiredTerms=concreteTerms(q,tokens,abstract,focus);
 const complex=/진로|수행평가|탐구|관심|연결|하지만|싫어|재미있|추천|관련|한국|국내|중국|일본|프랑스|영미|고전|재테크|인생|삶|철학/.test(q)||tokens.length>=5;
 return {query:q,tokens,material:uniq(material),genres:uniq(genres),topics:uniq(topics),emotions:uniq(emotions),origin,classicRequired,focus,wantsAuthor,complex,philosophyTradition,requiredTerms,explicitConcepts,compoundConcepts,softKeywords:[],softTopics:[]};
}
...
export function scoreRecord(rec,intent){
 ...
 if(intent.classicRequired){if(!materialMatch(rec,'소설'))return null;if(rec.classicMeta)return null;if(classicScore(rec)<2.2)return null;score+=13+Math.min(7,classicScore(rec));reasons.push('고전 소설 작품');}

 // 'A와 B', 'A과 B'처럼 두 개념을 직접 연결한 검색은 둘 다 실제 근거가 있어야 한다.
 for(const c of intent.explicitConcepts||[]){const m=explicitConceptMatch(rec,c);if(intent.compoundConcepts&&!m.ok)return null;if(m.ok){score+=Math.min(14,5+m.ev.score);reasons.push(c.name);}}
...
export function auditRecordAgainstIntent(rec,intent){
 const problems=[];if(intent.material.length&&!intent.material.every(m=>materialMatch(rec,m)))problems.push('자료형');if(intent.origin&&recOriginScore(rec,intent.origin)<3)problems.push('문학권');if(intent.philosophyTradition&&!philosophyMatch(rec,intent.philosophyTradition))problems.push('철학권역');if(intent.classicRequired&&classicScore(rec)<2.2)problems.push('고전근거');for(const t of intent.requiredTerms||[]){const ev=recTextEvidence(rec,[t],{history:isHistoryQuery(intent)});if(ev.titleHits+ev.introHits+ev.tocHits===0)problems.push(`필수어:${t}`);}if(intent.compoundConcepts){for(const c of intent.explicitConcepts||[]){if(!explicitConceptMatch(rec,c).ok)problems.push(`복합개념:${c.name}`);}}return problems;
}
