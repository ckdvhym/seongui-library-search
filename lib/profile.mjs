...
function buildConceptGroups(q,{material=[],requiredTerms=[],major=null,explicitConcepts=[]}={}){
 ...
 if(out.length===2){const a=cleanText(out[0].name),b=cleanText(out[1].name),raw=cleanText(q);const adjacent=new RegExp(`${escRx(a)}\\s+${escRx(b)}`).test(raw)||new RegExp(`${escRx(b)}\\s+${escRx(a)}`).test(raw);if(adjacent)return [{name:`${a} ${b}`,direct:[`${a} ${b}`,`${a}${b}`],topic:null,source:'phrase'}];}
