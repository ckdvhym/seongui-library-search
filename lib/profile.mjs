...
export function interpretLocal(query){
 ...
 if(/힐링|마음\s*(치유|회복)|지친\s*마음/.test(q))focus.push({name:'힐링·위로',terms:['위로','회복','치유','따뜻','희망','상처','마음','외로움','불안','슬픔','관계','가족','친구','사랑','새로운 시작','다시 시작','성장','용기','일상','휴식'],strict:false,semantic:true});
 ...
 if(/학교\s*(배경|에서|생활)|교내\s*(배경|생활)|학원물|고등학교\s*(배경|생활)/.test(q))focus.push({name:'학교 배경',terms:['학교','고등학교','교실','학생','동급생','친구','선생님','교내','학창시절','학교생활'],strict:true});
...
function explicitGenreMatch(rec,g){
 // 인덱스의 장르 태그 하나만 믿지 않고, 사용자가 장르를 직접 검색했을 때는 책 소개의 실제 근거도 확인한다.
 // 특히 로맨스는 모험/무협 속 부수적 연애 요소가 장르 전체로 오인되는 것을 막는다.
 if(g==='로맨스'){
  const strong=recTextEvidence(rec,['로맨스','연애','연애소설','첫사랑','러브스토리','사랑 이야기','사랑이야기']);
  if(strong.titleHits>0||strong.introHits>0)return true;
  const rel=recTextEvidence(rec,['사랑','연인','결혼','남녀','관계']);
  return rel.introHits>=2&&rel.distinct>=2;
 }
...
export function scoreRecord(rec,intent){
 ...
 for(const f of intent.focus||[]){const ev=recTextEvidence(rec,f.terms,{history:false});if(f.major){const core=recTextEvidence(rec,f.coreTerms||f.terms,{history:false});const coreOk=core.titleHits>0||core.introHits>0||(core.distinct>=2&&core.score>=3.2);if(!coreOk)return null;score+=Math.min(20,8+core.score);reasons.push(f.name);continue;}if(f.strict&&!(ev.titleHits>0||ev.introHits>=2||ev.distinct>=2||ev.score>=7))return null;if(f.semantic){if(ev.titleHits+ev.introHits===0&&ev.distinct<2&&ev.score<3)return null;score+=Math.min(17,5+ev.score);reasons.push(f.name);}else{score+=Math.min(15,5+ev.score);reasons.push(f.name);}}
