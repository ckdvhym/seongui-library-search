...
export function interpretLocal(query){
 ...
 if(/수의사|수의학/.test(q))focus.push({name:'수의학·수의사',terms:['수의사','수의학','동물병원','동물 의료','동물의료','동물 진료','동물진료','반려동물 진료','가축 질병','동물 질병'],strict:true});
 const origin=queryOrigin(q),classicRequired=/(^|\s)고전($|\s)|고전소설|고전 소설|고전문학|고전 문학|고전\s*(한국|세계)\s*문학/.test(q),classicTarget=/고전\s*소설|고전소설/.test(q)?'소설':(/고전.*문학|문학.*고전/.test(q)?'문학':null),wantsAuthor=/작가|저자|소설가|시인/.test(q);
 ...
 return {query:q,tokens,material:uniq(material),genres:uniq(genres),topics:uniq(topics),emotions:uniq(emotions),origin,classicRequired,classicTarget,focus,wantsAuthor,complex,philosophyTradition,requiredTerms,explicitConcepts,compoundConcepts,softKeywords:[],softTopics:[]};
}
...
export function scoreRecord(rec,intent){
 ...
 if(intent.classicRequired){if(intent.classicTarget==='소설'&&!materialMatch(rec,'소설'))return null;if(intent.classicTarget==='문학'&&!isLiterary(rec))return null;if(!intent.classicTarget&&!isLiterary(rec))return null;if(rec.classicMeta)return null;const minClassic=(intent.origin==='한국'&&intent.classicTarget==='문학')?1.5:2.2;if(classicScore(rec)<minClassic)return null;score+=13+Math.min(7,classicScore(rec));reasons.push(intent.classicTarget==='소설'?'고전 소설 작품':'고전 문학 작품');}
...
export function auditRecordAgainstIntent(rec,intent){
 const problems=[];if(intent.material.length&&!intent.material.every(m=>materialMatch(rec,m)))problems.push('자료형');if(intent.origin&&recOriginScore(rec,intent.origin)<3)problems.push('문학권');if(intent.philosophyTradition&&!philosophyMatch(rec,intent.philosophyTradition))problems.push('철학권역');if(intent.classicRequired){const minClassic=(intent.origin==='한국'&&intent.classicTarget==='문학')?1.5:2.2;if(classicScore(rec)<minClassic)problems.push('고전근거');if(intent.classicTarget==='소설'&&!materialMatch(rec,'소설'))problems.push('고전자료형');if(intent.classicTarget==='문학'&&!isLiterary(rec))problems.push('고전자료형');}for(const t of intent.requiredTerms||[]){const ev=recTextEvidence(rec,[t],{history:isHistoryQuery(intent)});if(ev.titleHits+ev.introHits+ev.tocHits===0)problems.push(`필수어:${t}`);}for(const c of intent.explicitConcepts||[]){if((intent.compoundConcepts||c.required)&&!explicitConceptMatch(rec,c).ok)problems.push(`${c.required?'필수개념':'복합개념'}:${c.name}`);}return problems;
}
