...
const EXPLICIT_CONCEPTS = [
 ...
 // 인물명 자체를 검색했을 때 Gemini의 넓은 철학 연상어로 퍼지지 않도록 핵심 저작·전통만 로컬에서 고정한다.
 {name:'공자', query:[/공자/], terms:['공자','논어','유교','유학','공자 사상','공자사상'], required:true,wholeTerms:['공자','논어','유교','유학']}
];
...
export function auditRecordAgainstIntent(rec,intent){
 const problems=[];if(intent.material.length&&!intent.material.every(m=>materialMatch(rec,m)))problems.push('자료형');if(intent.classicKorean&&!isKoreanClassic(rec))problems.push('한국고전근거');else if(intent.origin&&!originPass(rec,intent.origin))problems.push('문학권');if(intent.philosophyTradition&&!philosophyMatch(rec,intent.philosophyTradition))problems.push('철학권역');if(intent.classicRequired){const minClassic=intent.classicKorean?0.8:((intent.origin==='한국'&&intent.classicTarget==='문학')?1.5:2.2);if(classicScore(rec)<minClassic&&!intent.classicKorean)problems.push('고전근거');if(intent.classicTarget==='소설'&&!materialMatch(rec,'소설'))problems.push('고전자료형');if(intent.classicTarget==='문학'&&!isLiterary(rec))problems.push('고전자료형');}for(const t of intent.requiredTerms||[]){const ev=recTextEvidence(rec,[t],{history:isHistoryQuery(intent)});if(ev.titleHits+ev.introHits+ev.tocHits===0)problems.push(`필수어:${t}`);}for(const c of intent.explicitConcepts||[]){if((intent.compoundConcepts||c.required)&&!explicitConceptMatch(rec,c).ok)problems.push(`${c.required?'필수개념':'복합개념'}:${c.name}`);}if(intent.semanticFallback&&!semanticExpansionMatch(rec,intent).ok)problems.push('AI의미근거');return problems;
}
